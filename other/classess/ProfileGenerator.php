<?php
require_once("other/classess/ProfileData.php");
class ProfileGenerator{
    private $con, $usernameLoggedInObj, $profileData;

    public function __construct($con, $usernameLoggedInObj, $profileUsername){
        $this->con = $con;
    ...
    public function createContentSection(){
        ...
        if(sizeof($videos) > 0){
            $videoGrid = new VideoGrid($this->con, $this->usernameLoggedInObj);
    ...
    private function createHeaderButton(){
        if($this->usernameLoggedInObj->getUsername()==$this->profileData->getProfileUsername()){
            return "";
        }
        else{
